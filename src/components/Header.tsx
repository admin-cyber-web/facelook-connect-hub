...
const SearchModal = ({
  ...
  const handleAddFriend = async (e: React.MouseEvent, personId: string) => {
    ...
    if (!(await canViewPrivateProfile(personId))) {
  ...
  const doSearch = useCallback(
    async (q: string) => {
      ...
      try {
        const like = `%${q.trim()}%`;
        const [
          { data: p, error: peopleError },
          { data: c },
          { data: hp },
          { data: ps },
        ] = await Promise.all([
          searchVisibleProfiles(q, 20),
          supabase
            .from("groups")
        ...
        if (peopleError) {
          console.warn("[Header] profile search RPC failed:", peopleError);
        }
      ...
      <AnimatePresence>
        {newNotifPreview && !showNotif && (
          <motion.div
            ...
            className="fixed right-3 z-[120] w-[calc(100vw-24px)] max-w-[340px] rounded-2xl overflow-hidden shadow-2xl cursor-pointer"
            style={{ top: "calc(var(--cap-safe-top) + 16px)", background: "rgba(12,12,24,0.97)", border: "1px solid rgba(239,68,68,0.4)", backdropFilter: "blur(20px)" }}
            onClick={() => {
              setShowNotif(true);
              setHasNewNotif(false);
              setNewNotifPreview(null);
          ...
          <>
            ...
            >
              ...
              <div
                ...
                style={{ paddingTop: "calc(var(--cap-safe-top) + 12px)" }}
              >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-2xl p-4 border border-teal-500/20" style={{ background: "rgba(20,184,166,0.10)" }}>
                        <p
                            ...
                            style={{ "--perf-duration": "2.5s", "--perf-scale": "1.06", textShadow: "0 0 24px rgba(20,184,166,0.5)" } as React.CSSProperties}
                        >
                          {dashMagnetSent}
                        </p>
                      ...
                      <div className="rounded-2xl p-4 border border-purple-500/20" style={{ background: "rgba(124,58,237,0.10)" }}>
                        <p
                          ...
                          style={{ "--perf-duration": "2.5s", "--perf-scale": "1.06", "--perf-delay": "0.4s", textShadow: "0 0 24px rgba(124,58,237,0.5)" } as React.CSSProperties}
                        >
                          {dashMagnetReceived}
                        </p>
