...
const ChatSystem: React.FC<ChatSystemProps> = ({
  ...
  const [lpSearchResults, setLpSearchResults] = useState<SearchableProfile[]>([]);
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      if (allIds.size > 0) {
        const { data } = await supabase
          .from("profiles")
          .select("id, full_name, username, avatar_url, last_seen, is_private_mode")
          .in("id", [...allIds]);
        profiles = data || [];
      }

      const privateNonFriends = profiles.filter(
        (profile) => profile.is_private_mode === true && !friendIdSet.has(profile.id),
      );
      const privateAccessResults = await Promise.all(
        privateNonFriends.map(async (profile) => ({
          id: profile.id,
          allowed: await canViewPrivateProfile(profile.id),
        })),
      );
      const privateAllowedIds = new Set(
        privateAccessResults
      ...
      for (const p of profiles) {
        ...
        if (
          p.is_private_mode === true &&
          !friendIdSet.has(p.id) &&
          !privateAllowedIds.has(p.id)
        ) {
          continue;
        }
        const c: ChatContact = {
          ...
          is_private_mode: p.is_private_mode === true,
          ...(contactMap.get(p.id) || {}),
    ...
    const handler = async (e: Event) => {
      ...
      if (!target) {
        const { data: profile, error } = await supabase
          .from("profiles")
          .select("id, full_name, username, avatar_url, is_private_mode")
          .eq("id", detail.userId)
          .maybeSingle();
        if (error || !profile) {
        ...
        target = {
          id: profile.id,
          full_name: profile.full_name || profile.username || detail.full_name || "User",
          username: profile.username || "",
          avatar_url: profile.avatar_url || detail.avatar_url || "",
          is_private_mode: profile.is_private_mode === true,
          unread_count: 0,
        };
      }
      await handleSelectContact(target);
    };
  ...
  useEffect(() => {
    ...
    searchDebounceRef.current = setTimeout(async () => {
      ...
      const { data, error } = await searchVisibleProfiles(q, 20);
  ...
  const sendFriendRequest = async (targetId: string) => {
    ...
    try {
      // Re-check the same server-side visibility boundary before mutating.
      // This prevents a stale search result or crafted client from sending a
      // request to a private profile that is no longer visible to this user.
      if (!(await canViewPrivateProfile(targetId))) {
        toast.error("This private profile is unavailable.");
        return;
      }
  ...
  // ── Open chat ─────────────────────────────────────────────────────────────
  const handleSelectContact = async (user: ChatContact) => {
    ...
    if (
      user.is_private_mode === true &&
      friendship?.status !== "accepted" &&
      !(await canViewPrivateProfile(user.id))
    ) {
      toast.error("This private profile is only available to friends or mutual connections.");
      return;
    }
  ...
  const handleSelectFromSearch = (user: Profile) => {
    ...
    void handleSelectContact({ ...user } as ChatContact);
  };
  ...
  const sendMessage = async (overrideText?: string) => {
    ...
    if (
      selectedUser.is_private_mode === true &&
      friendshipMap.get(selectedUser.id)?.status !== "accepted" &&
      !(await canViewPrivateProfile(selectedUser.id))
    ) {
      toast.error("You can only message private profiles as a friend or mutual connection.");
          ...
          <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            {bottomTab === "chat" && !selectedUser && (
              <div className={`flex min-h-0 flex-1 flex-col overflow-hidden`}>
                {/* Header */}
                <div
                  className={`flex items-center gap-2 px-4 py-3 border-b ${T.divider} shrink-0`}
                >
                  <div className="min-w-0 shrink-0">
                    ...
                    <p className={`text-[10px] font-semibold ${T.text3}`}>
                  ...
                  <div className="relative min-w-0 flex-1">
                    <Search
                      size={13}
                      className={`absolute left-2.5 top-1/2 -translate-y-1/2 ${T.text3}`}
                    />
                    <input
                      type="text"
                      ...
                      className={`h-9 w-full rounded-xl pl-8 pr-8 text-xs font-semibold outline-none border ${T.searchBg}`}
                    />
                    {searchQuery && (
                      <button
                        type="button"
                        ...
                        className={`absolute right-2 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-white/10 flex items-center justify-center ${T.text3}`}
                      >
                        <X size={11} />
                      </button>
                    )}
                  ...
                  <button
                    ...
                    className={`w-8 h-8 rounded-xl bg-white/10 border ${T.divider} flex items-center justify-center ${T.text3} shrink-0`}
                  >
                    <X size={15} />
                  </button>
                </div>
                ...
                {/* List */}
                <div className="chat-scroll-pane flex min-h-0 flex-1 flex-col overflow-y-auto">
                  {searchQuery.trim() ? (
                    <>
                      <p
            ...
            {bottomTab === "chat" && selectedUser && (
              <div className={`relative flex min-h-0 flex-1 flex-col overflow-hidden`}>
                ...
                <div
                  ref={messagesPaneRef}
                  className="chat-scroll-pane flex min-h-0 flex-1 flex-col items-stretch overflow-y-auto px-4 pt-20 pb-4 space-y-2"
                  onClick={() => {
                    setMsgMenuId(null);
                    setShowEmojiGrid(false);
                    setShowInputEmoji(false);
                    ...
                    return filteredMessages.map((msg) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className={`flex w-full ${isMine ? "justify-end" : "justify-start"} group relative select-none`}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  lpSearchRef.current = setTimeout(async () => {
                                    const { data } = await searchVisibleProfiles(q, 6);
