...
// Engagement RPCs/views have existed with several names over time. Keep the
// component's internal contract stable: every post/reel uses likes_count.
// IDs from Supabase are UUID strings, but String() also keeps aggregate rows
// and cached rows aligned if a caller supplies a UUID-like value object.
const postIdKey = (id: unknown) => (id == null ? "" : String(id));

const totalLikesFrom = (row: any, fallback = 0) => {
  const raw = row?.total_likes ?? row?.likes_count ?? row?.like_count ?? fallback;
  const parsed = Number(raw);
  return Number.isFinite(parsed) && parsed >= 0 ? parsed : fallback;
};

function normalizePostLikeCount<T extends Record<string, any>>(
  post: T,
  ...
  return {
    ...post,
    likes_count: totalLikesFrom(post),
    surpriseMsg: post.surpriseMsg ?? metadata?.surpriseMsg ?? null,
  };
}
...
function getSurpriseMessage(post: any): SurpriseMessage | null {
  let raw = post?.surpriseMsg ?? post?.metadata?.surpriseMsg;
  ...
  if (!raw || typeof raw !== "object" || !raw.targetUserId) return null;
  return {
    targetUserId: String(raw.targetUserId),
    targetUserName: String(raw.targetUserName || "friend"),
    customMessage: String(raw.customMessage || ""),
    gifUrl: String(raw.gifUrl || ""),
    isSeen: raw.isSeen === true,
  };
}

const SurpriseReactionModal = ({
  message,
  onClose,
}: {
  ...
  return createPortal(
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100000] flex items-center justify-center overflow-hidden bg-[#12051c]/90 px-5 py-8 backdrop-blur-xl"
      onClick={onClose}
    >
      <motion.div
        ...
        className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-fuchsia-300/30 bg-gradient-to-br from-[#43105f] via-[#241040] to-[#110b28] p-1 shadow-[0_0_80px_rgba(217,70,239,0.35)]"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="relative overflow-hidden rounded-[1.8rem] bg-black/20 px-6 pb-7 pt-8 text-center">
          <div className="pointer-events-none absolute -left-16 -top-20 h-48 w-48 rounded-full bg-fuchsia-400/20 blur-3xl" />
          <div className="pointer-events-none absolute -bottom-24 -right-14 h-56 w-56 rounded-full bg-violet-400/20 blur-3xl" />

          <button
            type="button"
            onClick={onClose}
            aria-label="Close surprise message"
            className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-white/10 text-white/70 transition hover:bg-white/20 hover:text-white"
          >
            <X size={17} />
          </button>

          <div className="relative mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl border border-fuchsia-200/30 bg-gradient-to-br from-fuchsia-400 to-violet-500 text-3xl shadow-lg shadow-fuchsia-500/30">
            🎁
          </div>
          <p className="relative text-[11px] font-black uppercase tracking-[0.24em] text-fuchsia-200/80">
            A surprise from a friend
          </p>
          <h2 className="relative mt-2 text-2xl font-black text-white">
            Hey {message.targetUserName}! ✨
          </h2>

          {message.gifUrl && (
            <div className="relative mx-auto mt-5 max-h-56 overflow-hidden rounded-2xl border border-white/15 bg-black/20">
              <img
                src={message.gifUrl}
                alt="Surprise GIF"
          ...
          <p className="relative mt-5 whitespace-pre-wrap text-lg font-bold leading-relaxed text-white/90">
            {message.customMessage}
          </p>
          <p className="relative mt-5 text-[11px] font-semibold text-white/40">
            This surprise closes automatically in 4 seconds
          </p>
          <motion.div
            initial={{ scaleX: 1 }}
            animate={{ scaleX: 0 }}
            transition={{ duration: 4, ease: "linear" }}
            className="relative mt-4 h-1 origin-left rounded-full bg-gradient-to-r from-fuchsia-400 to-violet-400"
          />
        </div>
      </motion.div>
    </motion.div>,
...
const SingleReelBlock = ({
  ...
  // Keep a mounted reel in sync if its parent feed receives a refreshed
  // aggregate row while the reel is on screen.
  useEffect(() => {
    setLikeCount(totalLikesFrom(post));
  }, [post.id, post.likes_count, post.total_likes]);
...
const FameFeed = ({
  ...
  const [posts, setPosts] = useState<any[]>(() =>
    (cachedPosts?.data ?? []).map(normalizePostLikeCount),
  );
  ...
  const [surprisePopup, setSurprisePopup] = useState<SurpriseMessage | null>(null);
  const closeSurprisePopup = useCallback(() => setSurprisePopup(null), []);
  ...
  const [trendingFlicks, setTrendingFlicks] = useState<any[]>(
    () => (cachedFlicks?.data ?? []).map(normalizePostLikeCount),
  );
  ...
  // A successful CreatePost mutation publishes its inserted row locally so
  // the author's feed updates before the composer finishes its cleanup.
  useEffect(() => {
    const handleCreatedPost = (event: Event) => {
      const post = normalizePostLikeCount((event as CustomEvent<any>).detail ?? {});
      if (!post?.id || (currentUserId && post.author_id !== currentUserId)) return;
      setPosts((previous) => {
        const next = [
          post,
          ...previous.filter((item) => postIdKey(item.id) !== postIdKey(post.id)),
        ];
  ...
  const fetchPostEngagement = async (postIds: string[], uid: string | null) => {
    ...
    const requestedIds = postIds.map(postIdKey).filter(Boolean);
    ...
    try {
      ...
      if (error) {
        ...
        const { data: likes, error: fallbackError } = await supabase
          ...
          .in("post_id", requestedIds);
        if (fallbackError) throw fallbackError;
        const counts: Record<string, number> = {};
        const liked = new Set<string>();
        const reactions: Record<string, string> = {};
        for (const row of likes || []) {
          const id = postIdKey(row.post_id);
          if (!requestedIdSet.has(id)) continue;
          counts[id] = (counts[id] || 0) + 1;
          if (uid && postIdKey(row.user_id) === postIdKey(uid)) {
            liked.add(id);
            if (row.reaction_type) reactions[id] = row.reaction_type;
          }
        }
        setLikedIds((prev) => {
        ...
        setPosts((prev) =>
          prev.map((post) =>
            counts[postIdKey(post.id)] === undefined
              ? post
              : { ...post, likes_count: counts[postIdKey(post.id)] },
          ),
        );
        return;
      }
      const rows = (data || []) as any[];
      const pageIds = new Set(requestedIds);
      const liked = new Set<string>();
      const reactions: Record<string, string> = {};
      const counts: Record<string, number> = {};
      ...
      rows.forEach((row) => {
        const id = postIdKey(row.post_id);
        if (!requestedIdSet.has(id)) return;
        // The deployed aggregate contract may return total_likes while the
        // older RPC returns likes_count. Prefer total_likes when present.
        counts[id] = totalLikesFrom(row);
        commentCounts[id] = Number(row.comments_count) || 0;
        shareCounts[id] = Number(row.shares_count) || 0;
        if (row.liked_by_me) liked.add(id);
        if (row.reaction_type) reactions[id] = row.reaction_type;
      });
      setLikedIds((prev) => {
        const next = new Set(prev);
      ...
      setPosts((prev) =>
        prev.map((post) =>
          counts[postIdKey(post.id)] === undefined
            ...
            : {
                ...post,
                likes_count: counts[postIdKey(post.id)],
                comments_count: commentCounts[postIdKey(post.id)],
                shares_count: shareCounts[postIdKey(post.id)],
              },
        ),
      );
    } catch (e) {
  ...
  const revealSurpriseMessage = async (post: any) => {
    if (!currentUserId) return;
    const surprise = getSurpriseMessage(post);
    if (
      !surprise ||
      surprise.targetUserId !== currentUserId ||
      surprise.isSeen ||
    ...
    const { data, error } = await supabase.rpc("mark_surprise_message_seen", {
      p_post_id: post.id,
    });
    if (error) {
      console.warn("[FameFeed] surprise message could not be marked seen:", error.message);
      toast.error("Surprise message could not be opened. Please try again.");
      return;
    }
    if (data === false) return;

    const seenMessage = { ...surprise, isSeen: true };
    setPosts((previous) =>
      previous.map((item) =>
        item.id !== post.id
          ? item
          : {
              ...
              metadata: {
                ...(item.metadata || {}),
                surpriseMsg: seenMessage,
              },
            },
      ),
    );
    setSurprisePopup(seenMessage);
  };
  ...
  const fetchPosts = async (reset = false) => {
    ...
    let rows = (data ?? []).map(normalizePostLikeCount);
        ...
        if (myChains?.length) {
          ...
          if (newPostIds.length) {
            ...
            if (chainPosts?.length) {
              ...
              const marked = chainPosts.map(p => ({
                ...normalizePostLikeCount(p),
                _viral_reach: true,
              }));
              rows = [...marked, ...rows];
            }
  ...
  const handleReact = async (post: any, reactionType: string) => {
    ...
    // Reveal immediately after the like is accepted. Count reconciliation is
    // intentionally kept below this so an unrelated count request cannot
    // delay the recipient's one-time surprise experience.
    if (reactionType === "like") {
      await revealSurpriseMessage(post);
    }
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {surprisePopup && (
          <SurpriseReactionModal
            message={surprisePopup}
            onClose={closeSurprisePopup}
          />
        )}
      </AnimatePresence>
