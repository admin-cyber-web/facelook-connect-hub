  ...
  return (
    <div className="w-full">
      ...
      <button
        ...
        className="w-full flex items-center justify-between py-3 px-3.5 bg-gradient-to-r from-[#F7FFF9] to-[#E9FBEF]/80 rounded-[1.1rem] border border-emerald-200/70 hover:from-[#F1FFF5] hover:to-[#E9FBEF] transition-colors shadow-[0_7px_20px_rgba(34,197,94,0.06)]"
      >
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#E9FBEF] text-emerald-600">
            <Sparkles size={16} />
          </span>
          <span className="text-sm font-bold text-[#172554]">
            Flicks AI Suggestions
          </span>
          {suggestions && (
        ...
        {panelExpanded && (
          ...
          >
            ...
            <div className="mt-2 flex items-center justify-between gap-2">
              <button
                onClick={collapse}
                className="flex items-center gap-1 text-[11px] font-bold text-slate-500 hover:text-emerald-700 transition-colors px-2 py-1 rounded-lg hover:bg-emerald-50"
              >
              ...
              <button
                ...
                disabled={loading}
                className="flex items-center gap-1 text-[11px] font-bold text-emerald-700 hover:text-emerald-800 transition-colors px-2 py-1 rounded-lg hover:bg-emerald-50 disabled:opacity-50"
              >
            ...
            {autoCaption && (
              <div className="mt-2 p-3.5 bg-gradient-to-r from-[#FFFDF5] to-amber-50 border border-amber-200 rounded-[1.1rem]">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <button
                    ...
                    className="text-[11px] font-bold bg-emerald-600 text-white px-3 py-1.5 rounded-lg hover:bg-emerald-700 active:scale-95 transition-all"
                  >
                    Use
                  </button>
                  <button
                    onClick={() => handleReplace(autoCaption)}
                    className="text-[11px] font-bold bg-white text-emerald-700 border border-emerald-200 px-3 py-1.5 rounded-lg hover:bg-emerald-50 active:scale-95 transition-all"
                  >
                    Replace
                  </button>
                  <button
            ...
            {!error && suggestions && (
              <div className="mt-2">
                ...
                <div className="mt-2 space-y-2 max-h-[50vh] overflow-y-auto pr-1">
                  ...
                  {activeTab === "captions" && (
                    <>
                      {suggestions.captions.slice(0, itemsToShow).map((caption, i) => (
                        <div
                          key={i}
                          className="p-3.5 bg-white border border-slate-200/80 rounded-[1.1rem] hover:border-emerald-200 transition-colors group shadow-[0_5px_16px_rgba(23,37,84,0.025)]"
                        >
                          <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <div className="flex items-center gap-1.5 mt-2 opacity-60 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="text-[10px] font-bold bg-emerald-50 text-emerald-700 px-2 py-1 rounded-md hover:bg-emerald-100"
                            >
                              Use
                            </button>
                            <button
                      ...
                      {suggestions.captions.length > 3 && (
                        <button
                          ...
                          className="w-full text-[11px] font-bold text-emerald-600 py-1 hover:text-emerald-700 transition-colors"
                        >
                  ...
                  {/* Hashtags Tab */}
                  {activeTab === "hashtags" && (
                      <div className="p-3.5 bg-white border border-slate-200/80 rounded-[1.1rem] shadow-[0_5px_16px_rgba(23,37,84,0.025)]">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex gap-1">
                          <button
                            ...
                            className="text-[10px] font-bold bg-emerald-600 text-white px-2.5 py-1 rounded-md hover:bg-emerald-700 disabled:bg-slate-200 disabled:text-slate-400 transition-colors"
                          >
                  ...
                  {activeTab === "hooks" && (
                    <>
                      {suggestions.engagementHooks.map((hook, i) => (
                        <div
                          key={i}
                          className="p-3.5 bg-white border border-slate-200/80 rounded-[1.1rem] hover:border-violet-200 transition-colors shadow-[0_5px_16px_rgba(23,37,84,0.025)]"
                        >
                          ...
                          <div className="flex items-center gap-1.5 mt-2 pl-6">
                            <button
                              ...
                              className="text-[10px] font-bold bg-violet-50 text-violet-600 px-2 py-1 rounded-md hover:bg-violet-100"
                            >
                              Add to Post
                            </button>
                            <button
